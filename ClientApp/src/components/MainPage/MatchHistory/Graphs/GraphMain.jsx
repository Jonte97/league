...
const blueColors = ["#b0dfe5", "#7703fc", "#1d00c4", "#588bae", "#03fca5"];
const redColors = ["#b1560f", "#7c0a02", "#b80f0a", "#fa8072", "#d2b55b"];
...
const GraphMain = (props) => {
  const options = [
    { id: "gold", title: "Total Gold" },
    { id: "xp", title: "Total experience points" },
    { id: "cs", title: "Total minions killed" },
  ];
  const [active, setActive] = useState(options[0]);
  const initialData = configureGraphData(
    props.data.participantFrames,
  ...
  const [candidates, setCandidates] = useState(
    getCandidatesArray(props.participantList, props.owner)
  );

  //*Array of timestamps to print if graph
  let timestamps = [];
  let x = props.data.participantFrames.find((obj) => {
    return obj.participantId === 1;
  });
  ...
  useEffect(() => {
    setTeamParticipants(
      getParticipantTeam(props.participantList, props.championList, candidates)
    );
  ...
  const updateChartSettings = (id) => {
    ...
    setBarOptions({
      ...barOptions,
      options: { title: { text: options[id].title } },
    });
          ...
          {teamParticipants && candidates
            ? teamParticipants.blue.map((participant, key) => (
                <li
                  ...
                  style={
                    candidates.find(
                      (obj) => obj.participantId === participant.participantId
                    ).active === false
                      ? inactiveParticipant
      ...
      <div>
        <ul className="graph-nav">
          <li className="graph-nav-listitem">
            <a style={navbarActive[0]} onClick={() => updateChartSettings(0)}>
              Total gold
            </a>
          </li>
          <li className="graph-nav-listitem">
            <a style={navbarActive[1]} onClick={() => updateChartSettings(1)}>
              Xp gained
            </a>
          </li>
          <li className="graph-nav-listitem">
            <a style={navbarActive[2]} onClick={() => updateChartSettings(2)}>
              Minions killed
            </a>
          </li>
          <li className="graph-nav-listitem">
            <a style={navbarActive[3]} onClick={() => updateChartSettings(3)}>
              Team gold
            </a>
          </li>
        </ul>
